...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (pwd === con_pwd) {
      ...
      const existingPhone = await createDB.findOne({ phone: phone });
      const existingEmail = await createDB.findOne({ email: email });
      ...
      `);
        ...
        if (name && email && phone && pwd) {
          if (strongPassword.test(pwd) && phoneNumberFormat.test(phone)) {
            const newUser = new createDB({ name, email, phone, pwd });
            await newUser.save();
